...
void FitData()
{
    ...
    // Read data from file
    std::ifstream file("/home/goncalo/LFEA/analysis/main/dist.txt");
    if (!file.is_open())
    {
    ...
    graph->SetTitle("Counts VS Distance");
    graph->GetXaxis()->SetTitle("Distance [cm]");
    graph->GetYaxis()->SetTitle("N [Counts]");
    ...
    TF1 *fitFunc = new TF1("fitFunc", "[0] / x*x + [1]");
